import { MessageEvent, PresenceEvent, ReactionEvent, ReplyEvent, TypingEvent, UnsendEvent } from "@/@types/event";
...
export class EventParser {
  ...
  public static parse(topic: string, data: any): AppEvent[] {
    let events: AppEvent[] = [];
    if (topic === '/t_ms' && data?.deltas) {
      for (const delta of data.deltas) {
        const res = this.parseDelta(delta);
        if (res) {
          if (Array.isArray(res)) events.push(...(res as AppEvent[]));
          else events.push(res as AppEvent);
        }
      }
    }
  ...
  private static parseDelta(delta: any): AppEvent | AppEvent[] | null {
